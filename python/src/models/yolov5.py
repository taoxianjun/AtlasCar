...
class YoloV5:
    ...
    def infer(self, img_bgr):
        ...
        for idx, class_id in enumerate(pred_all[:, 5]):
            # FIX 2026-08-27: original line was
            #   if float(pred_all[idx][4] < float(0.05)): continue
            # The float(bool) wrapper is redundant but does NOT invert the logic
            # (conf<0.05 -> 1.0 -> DROP, conf>=0.05 -> 0.0 -> KEEP). Since NMS
            # already filters at conf_thres=0.6, this check is effectively a no-op
            # either way. Rewritten to the clear, configurable form below.
            if pred_all[idx][4] < self.conf_threshold:
                continue
