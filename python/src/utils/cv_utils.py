...
def preprocess_image_yolov5(image, cfg, bgr2rgb=True):
    ...
    # NOTE: 必须用 float16。yolo.om 的输入锁定为 float16 (640x640x3x2=2457600 字节)，
    # 故 YOLO 预处理必须输出 float16 才能与 .om 输入尺寸匹配。注意：ais_bench 在 numpy>=2.0 下
    # 无法推断 float16 数组 dtype (报 "Could not infer dtype of numpy.float16")，因此 DK 端 numpy
    # 必须保持 <2.0 (已 pip install numpy==1.26.4)。请勿改回 float32，否则会触发
    # "size:4915200 needsize:2457600 not match" 的尺寸不匹配错误。
